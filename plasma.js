/* Plasma shader background (21st.dev Shader Builder) — vanilla WebGL port.
   Cursor interaction disabled; animation only. Pauses when hidden/offscreen. */
(function () {
  var canvas = document.getElementById('plasma-bg');
  ...
  var FRAG = [
    ...
    '  float v = sin(p.x*k + t) + sin(p.y*k*0.8 - t*0.7) + sin((p.x+p.y)*k*0.6 + t*0.5) + sin(length(p)*k*1.2 - t);',
  ...
  var U = {
    /* silver monotone ramp: near-black -> charcoal -> silver -> light */
    colors: [
      [0.043, 0.043, 0.051],
      [0.169, 0.173, 0.192],
      [0.780, 0.796, 0.824],
    ...
    colorCount: 4, scale: 1.5, intensity: 0.48, paramA: 0.5, warp: 0.0,
    detail: 2.4, contrast: 0.95, brightness: -0.38, saturation: 0.0,
    hue: 0.0, vignette: 0.6, blur: 0.016, grain: 0.32,
    seed: 7.0, rotate: 0.0, offsetX: 0.0, offsetY: 0.0, drift: 0.156,
    oklab: 0.0, timeScale: 0.86,
  ...
  var start = performance.now(), raf = 0, visible = true, inView = true;

  function resize() {
    var dpr = Math.min(window.devicePixelRatio || 1, 2);
    var rw = Math.max(1, Math.round(canvas.clientWidth * dpr));
    var rh = Math.max(1, Math.round(canvas.clientHeight * dpr));
    var ps = Math.min(1, Math.sqrt(2000000 / Math.max(1, rw * rh)));
    var w = Math.max(1, Math.round(rw * ps));
    var h = Math.max(1, Math.round(rh * ps));
  ...
  function play() { if (!raf && visible && inView && !reduce) raf = requestAnimationFrame(frame); }

  window.addEventListener('resize', function () { resize(); if (reduce) draw(performance.now()); });
  document.addEventListener('visibilitychange', function () {
    visible = document.visibilityState === 'visible';
    if (visible) play(); else if (raf) { cancelAnimationFrame(raf); raf = 0; }
  });
  if ('IntersectionObserver' in window) {
    new IntersectionObserver(function (e) {
      inView = e[0] ? e[0].isIntersecting : true;
      if (inView) play(); else if (raf) { cancelAnimationFrame(raf); raf = 0; }
    }).observe(canvas);
  }
